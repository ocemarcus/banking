...
import { CreditCard, Settings, ArrowDownCircle, LayoutDashboard } from 'lucide-react';
...
export function DashboardLayout({ user, onLogout }) {
  ...
  const [accountForm, setAccountForm] = useState({ accountType: 'pf', document: '' });
  ...
  const initialTransactions = [
    { id: 'TRX-001', type: 'Entrada', description: 'Salário', amount: 5800, date: '2026-03-05' },
    { id: 'TRX-002', type: 'Saída', description: 'Aluguel', amount: 1800, date: '2026-03-03' },
    { id: 'TRX-003', type: 'Saída', description: 'Supermercado', amount: 420, date: '2026-03-02' },
    { id: 'TRX-004', type: 'Entrada', description: 'Freelance', amount: 1100, date: '2026-03-01' },
    { id: 'TRX-005', type: 'Saída', description: 'Energia', amount: 260, date: '2026-02-28' },
  ];
  ...
  React.useEffect(() => {
    ...
    getAccount()
      ...
      .catch((err) => {
        setErrorAccounts(err instanceof Error ? err.message : 'Erro ao buscar contas');
      })
  ...
  React.useEffect(() => {
    ...
    getTransactions({
      ...
      .then((res) => {
        ...
        const pageFromApi = typeof res?.page === 'number' ? res.page : transactionsPage;
        const totalFromApi = typeof res?.total === 'number' ? res.total : apiTransactions.length;
      ...
      .catch((err) => {
        setErrorTransactions(err instanceof Error ? err.message : 'Erro ao buscar transações');
      })
  ...
  async function handleCreateAccount(event) {
    ...
    try {
      ...
      setErrorAccounts(err instanceof Error ? err.message : 'Erro ao criar conta');
    } finally {
  ...
  function renderPage() {
    if (activePage === 'account') {
      return (
        <>
          ...
          {errorAccounts && <div className="p-4 text-red-600">{errorAccounts}</div>}
          <AccountPage
            accounts={accounts}
            accountForm={accountForm}
            onFormChange={(key, value) => setAccountForm((prev) => ({ ...prev, [key]: value }))}
    ...
    if (activePage === 'transactions') {
      return (
        <>
          {loadingTransactions && <div className="p-4">Carregando transações...</div>}
          <TransactionsPage
  ...
  return (
    <div className="flex min-h-screen bg-slate-100">
      <aside className="flex w-64 flex-col border-r border-slate-200 bg-slate-50 px-4 py-6">
        <div className="px-2">
          <h2 className="text-sm font-semibold tracking-[0.25em] text-slate-500">BANKING</h2>
