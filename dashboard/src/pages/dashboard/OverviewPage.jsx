...
export function OverviewPage({ user, totals }) {
	...
	const [dashboard, setDashboard] = useState({
		balance: saldo,
		totalIn: totals.entradas,
		totalOut: totals.saidas,
	});
	...
	useEffect(() => {
		...
		getDashboard()
			.then((data) => {
				...
				setDashboard({
					balance: data.balance ?? saldo,
					totalIn: data.totalIn ?? totals.entradas,
					totalOut: data.totalOut ?? totals.saidas,
				});
			})
			.catch(() => {
			...
			<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
				...
				<Card>
					...
					<CardContent>
						<p className="text-2xl font-bold">
							R${" "}
							{Number(dashboard.totalIn).toLocaleString("pt-BR", {
				...
				<Card>
					...
					<CardContent>
						<p className="text-2xl font-bold">
							R${" "}
							{Number(dashboard.totalOut).toLocaleString("pt-BR", {
								minimumFractionDigits: 2,
							})}
						</p>
